...
test("decodeHtmlEntities decodifica vocales acentuadas y ñ", () => {
  assert.equal(decodeHtmlEntities("Corte Suprema confirm&oacute; fallo"), "Corte Suprema confirmó fallo");
...
test("decodeHtmlEntities conserva el texto cuando no hay entidad conocida", () => {
  assert.equal(decodeHtmlEntities("Sin entidades aqu\u00ed"), "Sin entidades aqu\u00ed");
...
test("stripHtml quita etiquetas y decodifica entidades combinadas", () => {
  assert.equal(
    stripHtml("<b>Corte Suprema</b> acogi&oacute; el recurso &amp; confirm&oacute; la sentencia"),
